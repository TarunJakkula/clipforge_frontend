...
const initialState: BrollState = {
  ...
  loadingFiles: false,
  errorFiles: null,
  breadcrumbs: [{ name: "Brolls", id: "root" }],
...
export const fetchBroll = createAsyncThunk(
  "brolls/fetchBroll",
  async (
    { parent_id, space_id }: { parent_id: string; space_id: string },
    { rejectWithValue }
    ...
    try {
      const response = await axiosInstance.get("/fetch_broll", {
...
const brollsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchBroll.pending, (state, action) => {
        if (!state.files[action.meta.arg.parent_id]) state.loadingFiles = true;
        state.errorFiles = null;
      })
      .addCase(
        fetchBroll.fulfilled,
        (
          state,
          action: PayloadAction<{ brolls: File[]; parent_id: string }>
        ) => {
          state.loadingFiles = false;
          state.files[action.payload.parent_id] = action.payload.brolls;
          state.errorFiles = null;
        }
      )
      .addCase(fetchBroll.rejected, (state, action) => {
        state.loadingFiles = false;
        state.errorFiles =
...
export default brollsSlice.reducer;
