...
export default function LoginForm() {
  ...
  const handleEmail = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (e.status === 422) toast.error("Invalid Mail", { duration: 4000 });
      else toast.error("Error Occured. Try again.", { duration: 4000 });
      setEmail("");
    } finally {
      setLoading(false);
    }
  ...
  const handleOtp = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      if (!otpIdRef.current) throw Error("No Otp Id Found");
      const response = await axiosInstance.post(`/verify_user`, {
        email,
        otp,
        otp_id: otpIdRef.current,
      });
      dispatch(
        setUser({ uid: response.data.user_id, email: response.data.email })
      );
  ...
  return (
    <div className="md:w-[500px] w-full px-10">
      <form
        ...
        onSubmit={emailSent ? handleOtp : handleEmail}
      >
        <span
          className={`${ZenKakuGothicAntiqueMedium.className} desktop-tagline text-left w-full text-neutral-50`}
        >
          Email :
        </span>
        <div className="w-full h-[72px] rounded-xl flex justify-between items-center px-5 gap-4 bg-neutral-50">
          ...
          {!emailSent ? (
            <input
              ...
              required
              autoComplete="on"
              className={`${ZenKakuGothicAntiqueMedium.className} desktop-tagline w-full focus:ring-0 focus:outline-none bg-neutral-50`}
              value={email}
            ...
            <span
              className={`${ZenKakuGothicAntiqueMedium.className} desktop-tagline w-full focus:ring-0 focus:outline-none`}
            >
              {email}
            </span>
          )}
            ...
            <OTPInput
              ...
              renderInput={(props) => (
                <input
                  ...
                  className={`${ZenKakuGothicAntiqueMedium.className} desktop-tagline text-center focus:ring-0 focus:outline-none text-neutral-900 rounded-xl`}
                />
              )}
              containerStyle={{
                display: "flex",
        ...
        <button
          ...
          } px-5 items-center w-full h-[72px] rounded-xl`}
        >
          {loading ? (
            <svg
      ...
      {emailSent && (
        <div className="w-full flex justify-between items-center p-2">
          <button
            title="Reset back"
            className={`${ZenKakuGothicAntiqueBold.className} text-rose-500 desktop-tagline`}
            onClick={handleReset}
          >
            {" "}
            Wrong Email?
          ...
          <button
            title="Resend otp to mail"
            className={`${ZenKakuGothicAntiqueBold.className} text-blue-400 desktop-tagline`}
