...
import Element from './_noTranscript/Element'
import Image from 'next/image';
import { emptyFolderBlueIcon } from '@/ui/icons';

export default function NoTranscript() {
    const activeSpace = useAppSelector((state: RootState) => state.spaces.activeSpace);
    const router = useRouter();
    const dispatch = useAppDispatch();
    const clips = useAppSelector((state: RootState) => state.clips.noTranscript.clips);
    const error = useAppSelector((state: RootState) => state.clips.noTranscript.error);
    const [aspectRatio, setAspectRatio] = useState<"longform" | "shortform">("longform");
    const lfCount = useAppSelector((state: RootState) => state.clips.noTranscript.longformCount);
    const sfCount = useAppSelector((state: RootState) => state.clips.noTranscript.shortformCount);

    useEffect(() => {
        if (error)
            toast.error('Error fetching clips with no transcript');
    }, [error])
    ...
    return (
        <div className='flex flex-col w-full h-full gap-5 relative'>
            {clips.length === 0 ? <span className={`w-full text-center ${ZenKakuGothicAntiqueBold.className} h-full flex flex-col justify-center items-center desktop-h5 uppercase p-10`}>
                <Image src={emptyFolderBlueIcon} alt='' className='w-14 h-14' />
                No Clips
            </span> : <>
                <div className='w-full flex justify-end items-center h-fit absolute bottom-0 right-0'>
