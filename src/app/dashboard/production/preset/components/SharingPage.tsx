...
export default function SharingPage({ title, setOpen, presetId, spaceId }: SharingScreenProps) {
    ...
    const handleSubmit = async () => {
        try {
            await axiosInstance.post("/grant_preset_access", { preset_id: presetId, space_id: spaceId, spaces: sharedSpaces });
    ...
    useEffect(() => {
        const fetchSpaces = async () => {
            try {
                setLoading(true);
                const { data } = await axiosInstance.get("/fetch_shared_spaces", { params: { preset_id: presetId, space_id: spaceId } });
    ...
    return (
        <div className={`flex flex-col bg-transparent rounded-2xl pb-16 p-3 m-5 max-h-[400px] overflow-y-auto ${ZenKakuGothicAntiqueBold.className} gap-5`}>
